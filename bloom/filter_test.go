...
func TestFilterInsertKey(t *testing.T) {
	...
	wif, err := dogutil.DecodeWIF(secret)
	if err != nil {
	...
	f.Add(dogutil.Hash160(wif.SerializePubKey()))
...
func TestFilterBloomMatch(t *testing.T) {
	...
	tx, err := dogutil.NewTxFromBytes(strBytes)
	if err != nil {
	...
	spendingTx, err := dogutil.NewTxFromBytes(spendingTxBytes)
	if err != nil {
		t.Errorf("TestFilterBloomMatch NewTxFromBytes failure: %v", err)
		return
	}
