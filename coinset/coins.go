...
func (cs *CoinSet) TotalValue() (value dogutil.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue dogutil.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue dogutil.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount dogutil.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue dogutil.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue dogutil.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue dogutil.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue dogutil.Amount, coins []Coin) (Coins, error) {
...
// dogutil.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() dogutil.Amount {
	return dogutil.Amount(c.txOut().Value)
}
